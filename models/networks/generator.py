...
class SPADEGenerator(BaseNetwork):
    ...
    def forward(self, input, input_sw, rgb_img, obj_dic=None):
        seg = input
        seg_sw = input_sw

        x = F.interpolate(seg_sw, size=(self.sh, self.sw))
        x = self.fc(x)

        style_codes = self.Zencoder(input=rgb_img, segmap=seg)


        x = self.head_0(x, seg_sw, style_codes, obj_dic=obj_dic)

        x = self.up(x)
        x = self.G_middle_0(x, seg_sw, style_codes, obj_dic=obj_dic)

        if self.opt.num_upsampling_layers == 'more' or \
           self.opt.num_upsampling_layers == 'most':
            x = self.up(x)

        x = self.G_middle_1(x, seg_sw, style_codes,  obj_dic=obj_dic)

        x = self.up(x)
        x = self.up_0(x, seg_sw, style_codes, obj_dic=obj_dic)
        x = self.up(x)
        x = self.up_1(x, seg_sw, style_codes, obj_dic=obj_dic)
        x = self.up(x)
        x = self.up_2(x, seg_sw, style_codes, obj_dic=obj_dic)
        x = self.up(x)
        x = self.up_3(x, seg_sw, style_codes,  obj_dic=obj_dic)
    ...
    def forward_styles(self, input_sw, style_codes, obj_dic=None):
        seg_sw = input_sw
