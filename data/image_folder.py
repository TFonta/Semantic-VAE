...
def get_pose(pose_txt, img_dir, label_dir, isTrain):

    images = []
    labels = []
    poses = []

    if os.path.isfile(pose_txt):
        with open(pose_txt, 'r') as f:
            img_plus_poses = f.read().splitlines()
            if isTrain == True:
                img_plus_poses = img_plus_poses[2:28002]
            else:
                img_plus_poses = img_plus_poses[28002:]

            for line in img_plus_poses:
                #print(line)
                img_name, p1, p2, p3 = line.split()
                ...
                poses.append([float(p1), float(p2), float(p3)])
